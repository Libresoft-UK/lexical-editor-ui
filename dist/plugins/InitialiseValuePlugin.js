...
function filterValidNodes(serializedNode, registeredNodes) {
    ...
    const filteredNode = { ...serializedNode };
    if (Array.isArray(serializedNode.children)) {
        filteredNode.children = serializedNode.children
            .map((child) => filterValidNodes(child, registeredNodes))
            .filter((child) => child !== null);
    }
    return filteredNode;
}
function filterEditorState(serializedEditorState, editor) {
...
export default function InitialiseValuePlugin({ src = null, debug = false }) {
